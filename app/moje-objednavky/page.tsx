...
import { ArrowLeft, Package, MapPin, Clock, Eye } from 'lucide-react'
      ...
      <div className="max-w-2xl mx-auto p-6">
        ...
        {!customer ? (
          <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
            ...
            <p className="text-gray-500 mb-4">Pre zobrazenie objednávok sa prihláste</p>
            ...
            {orders.map(order => (
              <div key={order.id} className="bg-white rounded-2xl p-6 shadow-sm">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <p className="text-sm text-gray-500">{new Date(order.created_at).toLocaleDateString('sk-SK')}</p>
                ...
                {order.status === 'assigned' && (
                  <Link href={`/sledovat/${order.id}`} className="flex items-center gap-1 text-blue-600 text-sm font-medium mt-4">
